...
const util: utilType = {
  ...
  title: function (title?: string) {
    title = title || 'vue3版本前后端分离快速开发平台';
    window.document.title = title;
  },
  oneOf: function (ele: string, targetArr: string[]) {
...
/* util.title = function (title?: string) {
  title = title || 'vue3版本前后端分离快速开发平台';
  window.document.title = title;
}; */
...
/* util.oneOf = function (ele: string, targetArr: string[]) {
  if (targetArr.indexOf(ele) >= 0) {
    return true;
  } else {
    return false;
